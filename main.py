...
def load_data(tokenizer: AutoTokenizer, args):
    ...
    try:
        ...
        train_data, dev_data, test_data = pickle.load(open(os.path.join(args.dir_processed_data_for_datamap, f"data_split_bs{args.BATCH_SIZE}.pkl"), "rb"))
        all_user_inputs, all_user_labels = pickle.load(open(os.path.join(args.dir_processed_data_for_datamap, "all_data.pkl"), "rb"))
    except:
        print("File Loading Error, try to re-create dataset....")
        user_input_train, user_labels_train = get_data_emotions(args.origin_train_file, args.dir_processed_data_for_datamap)
        user_input_dev, user_labels_dev = get_data_emotions(args.origin_dev_file, args.dir_processed_data_for_datamap)
        user_input_test, user_labels_test = get_data_emotions(args.origin_test_file, args.dir_processed_data_for_datamap)
        ...
        pickle.dump([train_data, dev_data, test_data], open(os.path.join(args.dir_processed_data_for_datamap, f'data_split_bs{args.BATCH_SIZE}.pkl'), "wb"))
        pickle.dump([all_user_inputs, all_user_labels], open(os.path.join(args.dir_processed_data_for_datamap, "all_data.pkl"), "wb"))
...
if __name__ == '__main__':
    ...
    for no_fold in range(K_fold):
        ...
        #     train_loss, train_acc, _ = batch_iteration(_k_fold_train_data, model, criterion, optimizer, "train", False)
            ...
            json.dump(train_res_dict, open(os.path.join(args.dir_processed_data_for_datamap, "train_res_dict_epoch{}.json".format(_epoch)), "w", encoding='utf-8'), indent=4)
